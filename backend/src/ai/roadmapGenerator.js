...
async function generateRoadmapData({ branch, goal, level, companies, currentYear }) {
  try {
    ...
    const systemMessage = new SystemMessage(
      ...
      'Example format: ' +
      '[{"semester":1,"milestones":[{"task":"Learn basic C++","category":"Technical skills","status":"pending"},{"task":"Create GitHub profile","category":"Portfolio building","status":"pending"},{"task":"DSA basics","category":"Placement readiness","status":"pending"}]}]'
    );
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Roadmap AI Timeout')), 15000)
    );
    ...
    console.error('Roadmap AI generation failed or timed out, returning fallback:', err.message);
    const fallback = [];
    for (let sem = 1; sem <= 8; sem++) {
      fallback.push({
        semester: sem,
        milestones: [
          { task: `Master Semester ${sem} core engineering coursework`, category: 'Technical skills', status: 'pending' },
          { task: `Build and document Semester ${sem} portfolio project`, category: 'Portfolio building', status: 'pending' },
          { task: `Practice technical interviews & coding challenges`, category: 'Placement readiness', status: 'pending' }
        ]
      });
    }
    return fallback;
